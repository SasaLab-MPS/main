...
void setMinimumTemperature(void)
{
    double xij, yij, zij, distance2;
    minimumTemperature.resize(NumberOfParticles);

    for (int i = 0; i < NumberOfParticles; i++)
    {
        if (position[i].particleType == GHOST || position[i].particleType == DUMMY_WALL)
            continue;
        minimumTemperature[i] = temperature[i];
        ...
        for (int k = 0; k < (int)neghPar.size(); k++)
        {
            ...
            if (distance2 >= Re2_forGradient)
                continue;
            if (minimumTemperature[i] > temperature[j])
            {
                minimumTemperature[i] = temperature[j];
            }
        }
    }
}