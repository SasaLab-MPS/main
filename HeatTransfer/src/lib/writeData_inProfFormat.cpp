...
void writeData_inProfFormat(void)
{
    ...
    {
      fprintf(fp, "%d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", position[i].particleType, position[i].x, position[i].y, position[i].z, velocity[i].x, velocity[i].y, velocity[i].z, pressure(i), numberDensity[i], temperature[i], heatFlux[i]);
    }
    fclose(fp);
    FileNumber++;
}