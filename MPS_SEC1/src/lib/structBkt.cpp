...
void structBkt(void)
{
    ...
    nBx = (int)((MAX[0] - MIN[0]) * DBinv) + 2; // 解析領域のx方向のバケット数
    nBy = (int)((MAX[1] - MIN[1]) * DBinv) + 2; // 解析領域のy方向のバケット数
    nBz = (int)((MAX[2] - MIN[2]) * DBinv) + 2; // 解析領域のz方向のバケット数
